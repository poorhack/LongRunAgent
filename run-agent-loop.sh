...
#   - AGENT_INSTRUCTIONS.md (更新后的Agent操作指南)
#
# 原架构问题:
#   1. 外部脚本循环调用claude CLI，每次都是新会话
#   2. 脚本直接修改passes=true，没有独立验证机制
#   3. 所有工作在同一context，容易导致AI腐化
#
# 新架构优势:
#   1. 主agent持久保持context
#   2. 每个功能使用独立subagent实现（worktree隔离）
#   3. 验证subagent独立验证，只有通过才标记passes
#   4. 修复循环机制：验证失败后自动修复重试
#
# 保留此脚本仅作为参考，不建议继续使用。
# 如需使用旧方式，请取消此废弃标记。
#
...
get_next_feature() {
    ...
    if command -v python3 &>/dev/null; then
        python3 - "$features_file" << 'PYEOF' 2>/dev/null
import json
import sys
try:
...
PYEOF
        ...
        python - "$features_file" << 'PYEOF' 2>/dev/null
import json
import sys
try:
    with open(sys.argv[1], 'r') as f:
